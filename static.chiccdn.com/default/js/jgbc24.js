...
var JGData = {
	...
	// visit_group
	type: ['all', 'category', 'searchRecommend', 'home', 'searchResult', 'navigation', 'product'],
	...
	storageData: function (data) {
		if(window.localStorage) {
			...
			localStorage.setItem('jgList', JSON.stringify(rdList));
		}
	},
	// 发送未完成的请求
	...
	init: function(){
		...
		$(document).on('mousedown', '[data-point-id]', function () {
			...
			if (me.type.indexOf(data.visitGroup) !== -1) {
				if ($(this)[0].localName == 'a') {
					if (this.search.indexOf('from') == -1) {
						this.href = this.href + (this.search ? '&' : '?') + 'from=nav';
					}
				}
				// 再次获取sess_id
				if (!me.C.sess_id) {
					var userData = (me.getCookie('__bguser') || '').split('|');
					me.C.sess_id = userData[1] || '';
					me.C.user_id = userData[2] || '';
				}
				// 生成唯一标识tw_id
				var unixStr = new Date().getTime(),
					href = $(this).attr('href') || '',
					tw_id = me.C.sess_id + unixStr;
				// 保存tw_id到localStorage
				if (window.nDC && window.nDC.keywordStorage) {
					window.nDC.keywordStorage.push({key:href,value:tw_id});
				}
				sendData.info.string4 = tw_id;
			}
			if ($(this)[0].localName == 'a' && $(this).href !='javascript:void(0);' && !/^#/.test($(this).href)) {
				unStorage = false;
			} 
			me.sendRec(sendData,unStorage);
		});

		$(document).on('mouseenter','[data-point-id-hover]',function(){
			if(typeof(JGData) != 'undefined'){
				var pointId = $(this).attr('data-point-id-hover');
				me.sendRec({ 'point_id': pointId, ac: 'hover' }, true);
			}
		})
	},
	...
	sendRec: function(extraParam, unStorage){
		...
		if(unStorage){
			...
			// 20200508
			var g = document.location.protocol + "//bi.banggood.com/index.php?com=sess&t=record" + s + "&callback=?";
			$.ajax({
				url:g,
				type:'GET',
				dataType:'json',
				complete:function(){
					JGData.removeLocalData();
				}
			})
		} else {
